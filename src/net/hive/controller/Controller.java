...
public class Controller implements Initializable{
    ...
    private static String t2 = "";      // Дата создания карточки
    private static String t21 = "";     // Дата возврата карточки
    ...
    static String urSite = null;

    //ControllerBD wayToDB = new ControllerBD(urSite);
    ...
    private Connection getConnection() throws SQLException, IOException {
                ...
                // Путь к рабочей БД
                //String urSite = wayToDB.getWayToDB();
                ...
                prop.setProperty("password", "!a234567");
    ...
    private void ifElseLogic(boolean vkladka){
        LocalDate d2;
        LocalDate e2;
        if (vkladka){
            d2 = dataDate.getValue();       // Вводим дату начала поиска
            e2 = dataDate1.getValue();      // Вводим дату кончала поиска :-)
        }else {
            d2 = dataDate2.getValue();      // Вводим дату начала поиска
            e2 = dataDate21.getValue();     // Вводим дату кончала поиска :-)
        }
        if (d2 == null){
            String dat = f.format(formatter);
            dataDate.setPromptText(dat);
    ...
    private void informationText(String contentText){
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Export complete");
        alert.setHeaderText("Файл успешно экспортирован");
        alert.setContentText(contentText);
        alert.showAndWait();
    }
    private void errorMessage(String contentText, String headerText){
    ...
    public void exExcelButton1() throws IOException, NullPointerException  {
        ...
        informationText("Файл называется: " + a);
    }
    ...
    public void exOtchetOFButton() throws IOException, NullPointerException  {
        pojoData.removeAll(pojoData);       // Чистим коллекцию от данных
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        Scanner sc = null;
        try {
            sc = new Scanner(new File("./queryUrlFB.txt"));
        } catch (FileNotFoundException e3) {
            e3.printStackTrace();
            errorMessage("Файл queryUrlFB.txt должен находится рядом с приложением","Файл не найден!");
        }
            String text = sc.next();
